...
Begin VB.Form frmSavings 
   BackColor       =   &H00008000&
   Caption         =   "Savings Transaction Posting"
   ClientHeight    =   6525
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   11760
   Icon            =   "frmDps.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6525
...
'----------------------------------------------------------------------------
    ...
    Set rs = New ADODB.Recordset
        str = "select * from Tran where cdate(Date) Between cdate('" & Today & "') And cdate('" & Today & "') Order by Sl Desc"
        rs.Open str, conn
...
On Error Resume Next
    txtInstallment.Text = Format$(Val(txtInstallment.Text), "###0.00")
    txtOther.Text = Format$(Val(txtOther.Text), "###0.00")
    txtTotal.Text = Format$(Val(txtInstallment.Text) + Val(txtOther.Text), "###0.00")
    txtNet.SelStart = 0
    txtNet.SelLength = Len(txtNet.Text)
End Sub
