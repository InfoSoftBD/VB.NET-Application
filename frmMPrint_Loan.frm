...
Begin VB.Form frmMPrint_Loan 
   BackColor       =   &H00008000&
   Caption         =   "Monthly Loan Statement"
   ClientHeight    =   6570
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   5595
   Icon            =   "frmMPrint_Loan.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6570
   ...
   Begin VB.PictureBox Picture2 
      ...
      Begin MSDataGridLib.DataGrid DataGrid1 
         Bindings        =   "frmMPrint_Loan.frx":0442
         Height          =   1920
         Left            =   135
         TabIndex        =   23
         Top             =   135
   ...
   End
End
Attribute VB_Name = "frmMPrint_Loan"
...
Set rs = New ADODB.Recordset
    If cmbTerm.Text = "All" Then
        str = "select * FROM Loan_Master where FO_Code like '" & FO & "' AND Samity_Code like '" & Samity & "' order by Sl_No"
        rptLoanMonthly.Label38.Caption = "MONTHLY LOAN COLLECTION REPORT ALL FOR THE MONTH OF" & Mont & "-" & yr
    Else
        str = "select * FROM Loan_Master where FO_Code like '" & FO & "' AND Samity_Code like '" & Samity & "' AND Term like '" & Trm & "' order by Sl_No"
        rptLoanMonthly.Label38.Caption = "MONTHLY LOAN COLLECTION REPORT TERM WISE FOR THE MONTH OF" & Mont & "-" & yr
    End If
...
Set rsN = New ADODB.Recordset
    ...
    Set rs = New ADODB.Recordset
    If cmbTerm.Text = "All" Then
    str = "select * FROM Loan_Print where FO_Code like '" & FO & "' AND Samity_Code like '" & Samity & "' order by Sl_No"
    Else
    str = "select * FROM Loan_Print where FO_Code like '" & FO & "' AND Samity_Code like '" & Samity & "' AND Term like '" & Trm & "' order by Sl_No"
    End If
    rs.Open str, conn
        
    If Not rs.EOF Then
        rptLoanMonthly.rsLoanMonthly.ConnectionString = cnStr
        rptLoanMonthly.rsLoanMonthly.Source = str
        Timer1.Enabled = True
    Else
        MsgBox "There is no such criteria found!, ", vbCritical + vbOKOnly
        rs.Close
...
If optCenter.Value = True Then
    ...
    Set rs = New ADODB.Recordset
    If cmbTerm.Text = "All" Then
    str = "select * FROM Loan_Master where Center_Code like '" & Center & "'order by Sl_No"
    Else
    str = "select * FROM Loan_Master where Center_Code like '" & Center & "'AND Term like '" & Trm & "'order by Sl_No"
    End If
        rs.Open str, conn
