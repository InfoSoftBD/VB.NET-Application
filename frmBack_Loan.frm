...
Begin VB.Form frmBack_Loan 
   BackColor       =   &H00008000&
   Caption         =   "Loan Old Data Posting"
   ClientHeight    =   5865
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   11010
   Icon            =   "frmBack_Loan.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   5865
   ...
   Begin VB.PictureBox Picture5 
      ...
      Begin MSDataGridLib.DataGrid DataGrid1 
         Bindings        =   "frmBack_Loan.frx":0442
         Height          =   4605
         Left            =   150
         TabIndex        =   34
         Top             =   735
   ...
   End
End
Attribute VB_Name = "frmBack_Loan"
...
Private Sub Command2_Click()
    ...
    If Not rsU.EOF Then
        rsU!Balance = -Val(txtNet.Text)
        rsU!Paid = rsU!Paid + (Val(rsU!Amount) - Val(txtNet.Text))
        rsU!Daily_Pay = rsU!Daily_Pay + (Val(rsU!Amount) - Val(txtNet.Text))
        rsU!Weekly_Pay = rsU!Weekly_Pay + (Val(rsU!Amount) - Val(txtNet.Text))
        rsU!Monthly_Pay = rsU!Monthly_Pay + (Val(rsU!Amount) - Val(txtNet.Text))
        rsU!Yearly_Pay = rsU!Yearly_Pay + (Val(rsU!Amount) - Val(txtNet.Text))
        rsU!Inst_Paid = rsU!Inst_no - (Val(txtNet.Text) / Val(txtInstallment.Text))
        rsU!Inst_Due = rsU!Inst_no - rsU!Inst_Paid
    ...
    Set rsN = New ADODB.Recordset
        ...
        rsN!Cr = (Val(rsU!Amount) - Val(txtNet.Text))
        'rsN!Fine = txtOther.Text
        rsN!Balance = rsU!Balance
        rsN!Advance = rsU!Advance
        rsN!Term = rsU!Term
        rsN!Type = rsU!Type
        rsN!Daily_Pay = (Val(rsU!Amount) - Val(txtNet.Text))
    ...
    If txtType.Text = "Product" Then
            ...
            rs!Balance = rs!Balance - (Val(rsU!Amount) - Val(txtNet.Text))
            rs.Update
      ...
      Set rs = New ADODB.Recordset
        ...
        Set rsN = New ADODB.Recordset
            ...
            rsN!Name = rs!Head_Name
            rsN!Description = "A/C:" & txtAccount.Text
            rsN!Dr = (Val(rsU!Amount) - Val(txtNet.Text))
            rsN!Cr = 0
            rsN!Balance = rs!Balance
            rsN.Update
            rsN.Close
        ...
        Set rsN = New ADODB.Recordset
            ...
            rsN.MoveFirst
            rsN!Cash_Dr = rsU!Cash_Dr + (Val(rsU!Amount) - Val(txtNet.Text))
            rsN!Cash_Close = rsU!Cash_Close + (Val(rsU!Amount) - Val(txtNet.Text))
            rsN.Update
            rsN.Close
            rsU.Close
        End If
...
Private Sub Command3_Click()
 Set rs = New ADODB.Recordset
    ...
    If Not rsU.EOF Then
        ...
        rsU!Inst_Due = rsU!Inst_no + (Val(txtNet.Text) / Val(txtInstallment.Text))
        rsU!Inst_Paid = rsU!Inst_no + rsU!Inst_Due
        rsU!Term_Fail = Val(txtDefaulter.Text)
        rsU!Due = Val(txtOverdue.Text)
        rsU.Update
...
Private Sub Form_Load()
 ...
 On Error Resume Next
    ...
    Set rs = New ADODB.Recordset
        str = "select * from Loan_Tran where cdate(Date) Between cdate('" & Today & "') And cdate('" & Today & "') Order by Sl"
        rs.Open str, conn
