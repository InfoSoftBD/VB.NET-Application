...
Begin VB.Form frmLoan_Posting 
   ...
   Begin VB.PictureBox Picture5 
      ...
      Begin MSDataGridLib.DataGrid DataGrid1 
         Bindings        =   "frmLoan_Posting.frx":08CA
         Height          =   5100
         Left            =   150
         TabIndex        =   5
         Top             =   735
   ...
   End
End
Attribute VB_Name = "frmLoan_Posting"
...
On Error Resume Next
    Dim Term As Integer
    Dim vn As String
    Term = Val(txtNet.Text) / Val(txtInstallment.Text)
    Set rsU = New ADODB.Recordset
        str = "select * from Customer_Master where Customer like '" & txtAccount.Text & "'"
        rsU.Open str, conn, adOpenDynamic, adLockOptimistic
            
If Not rsU.EOF Then
            
        If rsU!Due_Amnt < Val(txtNet.Text) Then
            rsU!Balance = rsU!Balance + Val(txtNet.Text)
            'rsU!Fine = rsU!Fine + Val(txtOther.Text)
            rsU!Inst_Date = txtDate.Text
            rsU!Cr = rsU!Cr + Val(txtNet.Text)
            rsU!Advance = rsU!Advance + Val(txtNet.Text)
            rsU!Inst_Paid = rsU!Inst_Paid + Term
        Else
            rsU!Balance = rsU!Balance + Val(txtNet.Text)
            'rsU!Fine = rsU!Fine + Val(txtOther.Text)
            rsU!Inst_Date = txtDate.Text
            rsU!Cr = rsU!Cr + Val(txtNet.Text)
            rsU!Due_Amnt = rsU!Due_Amnt - Val(txtNet.Text)
            rsU!Inst_Paid = rsU!Inst_Paid + Term
        End If
        
        If rsU!Default >= 1 Then
            rsU!Inst_Due = rsU!Inst_Due - Term
            rsU!Default = rsU!Default - Term
        Else
            rsU!Inst_Due = rsU!Inst_Due - Term
        End If

        
        If rsU!Term = "Weekly" Then
        rsU!Due_Date = DateAdd("D", 7, CDate(rsU!Due_Date))
        End If
        
        If rsU!Term = "Half Monthly" Then
        rsU!Due_Date = DateAdd("D", 15, CDate(rsU!Due_Date))
        End If
        
        If rsU!Term = "Monthly" Then
        rsU!Due_Date = DateAdd("M", 1, CDate(rsU!Due_Date))
        End If
        
        If rsU!Term = "Quarly" Then
        rsU!Due_Date = DateAdd("M", 3, CDate(rsU!Due_Date))
        End If
        
        If rsU!Term = "Yearly" Then
        rsU!Due_Date = DateAdd("YYYY", 1, CDate(rsU!Due_Date))
        End If
        rsU.Update
    ...
    Set rsN = New ADODB.Recordset
        ...
        rsN.AddNew
            ...
            rsN!Invoice = rsN!ID
            vn = rsN!ID
            rsN!Customer = txtAccount.Text
            rsN!Name = txtName.Text
            rsN!Type = rsU!Type
            rsN!Address = rsU!Address
            rsN!Mobile = rsU!Mobile
...
Set rsU = New ADODB.Recordset
    ...
    Set rs = New ADODB.Recordset
        str = "select * from Customer_Tran where cdate(Date) Between cdate('" & Today & "') And cdate('" & Today & "') Order by ID"
        rs.Open str, conn
...
Dim vn As String
 Set rs = New ADODB.Recordset
        str = "select * from Loan_Tran where sl like '" & txtMR_No.Text & "'"
        rs.Open str, conn
...
If Not rs.EOF Then
    On Error Resume Next
        ...
        txtTotal.Text = Val(rs!Fine) + Val(rs!Cr)
        txtBalance.Text = rs!Balance
        rs.Close
     
If MsgBox("Really want to delete?", vbCritical + vbYesNo) = vbYes Then
        
        str = "delete from Loan_Tran where sl like '" & txtMR_No.Text & "'"
        rs.Open str, conn, adOpenDynamic, adLockOptimistic
        rs.Close
    ...
    If Not rsU.EOF Then
        ...
        If rsU!Due <= Val(txtNet.Text) Then
            rsU!Due = rsU!Due + Val(txtNet.Text)
        End If
        
        If rsU!Inst_Due <= 0 Then
            Term = Val(txtNet.Text) / Val(txtInstallment.Text)
            rsU!Inst_Due = rsU!Inst_Due + Term
        End If
     
        
        If rsU!Term_Fail <= 1 Then
            Term = Val(txtNet.Text) / Val(txtInstallment.Text)
            rsU!Term_Fail = rsU!Term_Fail + Term
        End If
        ...
        If rsU!Term = "Daily" Then
           rsU!Due_Date = DateAdd("D", -1, CDate(rsU!Due_Date))
        End If
        ...
        If rsU!Term = "Yearly" Then
        rsU!Due_Date = DateAdd("Y", -1, CDate(rsU!Due_Date))
        End If
    ...
    If txtType.Text = "Product" Then
       ...
       Set rsN = New ADODB.Recordset
        ...
        Set rsN = New ADODB.Recordset
            ...
            rsN!AC_No = rs!AC_No
            rsN!Name = rs!Head_Name
            rsN!Description = "Fine Rev A/C:" & txtAccount.Text
...
End If
'----------------------------------------------------------------------
If txtType.Text = "Cash Advance" Then
            Set rs = New ADODB.Recordset
...
If MsgBox("Really want to Print?", vbCritical + vbYesNo) = vbYes Then
 ...
 Set rs = New ADODB.Recordset
    ...
    Set rsN = New ADODB.Recordset
        ...
        rsN!Name = txtName.Text
        rsN!Description = txtTerm.Text + " " + txtType.Text + " Loan " + txtDescription.Text + ", FO Code-" & rsU!FO_Code + ", FO Name:" & rsU!FO_Name
        rsN!Cr = txtNet.Text
        rsN!Fine = txtOther.Text
...
Dim ID As String
    ...
    Set rs = New ADODB.Recordset
        str = "select * from Customer_Master where Customer like '" & ID & "'"
        rs.Open str, conn
    ...
    If Not rs.EOF Then
        If rs!C_lose = "Yes" Then
                ...
                txtAccount.SetFocus
            Else
               If rs!Default > 1 Then
               MsgBox "Customer is Defaulter for " & rs!Default & "Term", vbCritical, "Information!"
               End If
               
               'Call clearTextboxes
                On Error Resume Next
        
                txtAccount.Text = rs!Customer
                txtDate.Text = Today
                txtName.Text = rs!Name
                txtType.Text = rs!Type
                txtTerm.Text = rs!Term
                txtDefaulter.Text = rs!Default
                txtOverdue.Text = rs!Due_Amnt
                txtInstallment.Text = rs!Inst_Amnt
                txtBalance.Text = rs!Balance
                txtNet.Text = txtInstallment.Text
                rs.Close
...
Dim Tran As String
    ...
    Set rs = New ADODB.Recordset
        str = "select * from Loan_Tran where sl like '" & Tran & "'"
        rs.Open str, conn
    ...
    On Error Resume Next
        ...
        txtNet.Text = rs!Cr
        txtOther.Text = rs!Fine
        txtTotal.Text = Val(rs!Cr) + Val(rs!Fine)
        txtBalance.Text = rs!Balance
        rs.Close
...
End If
    ...
    Set rs = New ADODB.Recordset
        str = "select * from Loan_Tran where Name Like '" & "%" & search & "%" & "'"
        rs.Open str, conn
